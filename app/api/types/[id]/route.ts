...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const body = await req.json() as Partial<{ name: string; slug: string | null; duration_minutes: number; price: number; is_active: boolean; is_public: boolean; sort_order: number }>;
