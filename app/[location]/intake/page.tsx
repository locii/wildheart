...
export default function IntakePage() {
  ...
  useEffect(() => {
    fetch("/api/intake")
      .then((r) => r.json())
      .then(({ questions }) => { setQuestions(questions ?? []); setLoading(false); });
  }, []);
  ...
  async function handleSubmit(skipped: boolean) {
    ...
    router.push(`/${locationSlug}/success?skipped=${skipped ? "1" : "0"}`);
  }

  if (loading) return (
    <BookingShell backHref="/" backLabel="Back to booking">
  ...
  if (questions.length === 0) return (
    <BookingShell backHref="/" backLabel="Back to booking">
      <div className="px-8 py-12 text-center">
        ...
        <Button onClick={() => router.push(`/${locationSlug}/success`)}>Continue</Button>
      </div>
    </BookingShell>
  );
